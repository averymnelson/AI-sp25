...
class CornersProblem(search.SearchProblem):
    ...
    def getStartState(self):
        ...
        """
        "*** YOUR CODE HERE ***"
        return (self.startingPosition, tuple(False for _ in self.corners), self.startingHitWalls)
    
    def isWall(self, state):
        "*** YOUR CODE HERE ***"
        # test_x, test_y = 3, 2  # Change this to a known wall coordinate
        # print(f"Wall at ({test_x}, {test_y})? {self.walls[test_y]}, {self.walls[test_y][test_x]}")
        # print(f"Type of self.walls[4]: {type(self.walls[4])}")
        # print(f"self.walls[4] raw output: {self.walls[4]}")
        x, y = state[0]
        # print(f"self.walls x and y: {x}, {y}, {self.walls[y][x]}")
        # print(f"self.walls[x][y]: {self.walls[x][y]}\n")
    ...
    def isGoalState(self, state):
        ...
        all_corners_visited = all(visited_corners)
        # Check if hit walls constraint is satisfied (at least 1, no more than 2)
        valid_wall_hits = hit_walls >= 1 and hit_walls <= 2
        return all_corners_visited and valid_wall_hits
    ...
    def getSuccessors(self, state):
        ...
        # If we've hit too many walls, don't explore further
        if hit_walls > 2:
            return successors
        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            # Check if the next position is valid (within maze bounds)
            if not (0 <= nextx < self.walls.width and 0 <= nexty < self.walls.height):
                continue
                
            # Check if next position is a wall
            next_position = (nextx, nexty)
            is_next_wall = self.walls[nextx][nexty]
            
            if is_next_wall:
                next_hit_walls = hit_walls + 1
                # If it's a wall and we already hit 2 walls, don't proceed
                if hit_walls >= 2:
                    continue
            else:
                next_hit_walls = hit_walls
                
            # Update visited corners if we're at a corner
            ...
            # Add the successor
            successors.append(
                ((next_position, tuple(next_visited_corners), next_hit_walls), 
                 action, 
                 self.costFn(next_position))
            )
        # print(f"Successor: {next_state}, Action: {action}")
        # print(f"Successors of {state}: {successors}")
        self._expanded += 1 # DO NOT CHANGE
        return successors

    def getCostOfActions(self, actions):
        """
        Returns the cost of a particular sequence of actions.  This is implemented for you.
        """
        if actions == None: return 999999 # Pacman doest not move
        return len(actions)
...
class AStarFoodSearchAgent(SearchAgent):
    "A SearchAgent for FoodSearchProblem using A* and your foodHeuristic"
    def __init__(self):
        self.searchFunction = lambda prob: search.aStarSearch(prob, heuristic=foodHeuristic)
...
def mazeDistance(point1, point2, gameState):
    ...
    return len(search.bfs(prob, initialHit=2)) ## Do not allow Pacman hit walls
